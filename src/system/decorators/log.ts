...
function setCorrelationContext(
...
export function Log<T extends (...arg: any) => any>(
    ...
    let logFn = (message: string, ...params: any[]) => {
        console.log(message || emptyStr, ...params);
    };

    if (Logger) {
        logFn = (options.debug
            ? Logger.debug.bind(Logger)
            : Logger.log.bind(Logger)) as
            | typeof Logger.debug
            | typeof Logger.log;
    }
        ...
        descriptor.value = function(this: any, ...args: Parameters<T>) {
            ...
            if (
                (!Container.isDebugging &&
                    Logger.level !== IOutputLevel.Debug &&
                    !(
                        Logger.level === IOutputLevel.Verbose && !options.debug
                    )) ||
                (typeof options.condition === 'function' &&
                    !options.condition(...args))
            ) {
            ...
            if (this != null) {
                ...
                if (
                    ...
                    this.constructor[LogInstanceNameFn]
                ) {
                    instanceName = target.constructor[LogInstanceNameFn](
                        this,
                        instanceName
                    );
                }
            ...
            let prefix = `${
                correlate ? `[${correlationId.toString(16)}] ` : emptyStr
            }${instanceName ? `${instanceName}.` : emptyStr}${key}`;
            ...
            const enter =
                options.enter != null ? options.enter(...args) : emptyStr;
            ...
            if (options.args === false || args.length === 0) {
                ...
                const argFns =
                    typeof options.args === 'object' ? options.args : undefined;
                let argFn;
                let loggable;
                loggableParams = args
                    .map((v: any, index: number) => {
                        ...
                        argFn =
                            argFns !== undefined ? argFns[index] : undefined;
                        if (argFn !== undefined) {
                            loggable = argFn(v);
            ...
            if (options.singleLine || options.timed || options.exit != null) {
                ...
                const logError = (ex: Error) => {
                    const timing =
                        start !== undefined
                            ? ` \u2022 ${Strings.getDurationMilliseconds(
                                  start
                              )} ms`
                            : emptyStr;
                    if (options.singleLine) {
