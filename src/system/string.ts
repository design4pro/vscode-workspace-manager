...
export namespace Strings {
    ...
    export function padOrTruncate(
        ...
        if (width < max)
            return left
                ? padLeft(s, max, padding, width)
                : padRight(s, max, padding, width);
        if (width > max) return truncate(s, max, undefined, width);
        return s;
    }
    ...
    export function pluralize(
        ...
        return `${
            count === 0 ? options.zero || count : options.number || count
        } ${
    ...
    export function splitSingle(s: string, splitter: string) {
        ...
        const first = parts[0];
        return first.length === s.length
            ? parts
            : [first, s.substr(first.length + 1)];
    }
